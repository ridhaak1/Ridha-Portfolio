...
const springTrans = (delay = 0, duration = 0.55): Transition => ({ duration, ease: SPRING, delay });
const fadeUpTrans = (delay = 0): Transition => ({ duration: 0.55, ease: EASE_OUT, delay });
...
const letterVariants = (i: number): Variants => ({
  ...
  visible: { opacity: 1, y: 0, skewX: 0, transition: springTrans(0.42 + i * 0.09, 0.6) },
});
...
export default function Hero() {
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (photoRef.current) photoRef.current.style.transform = `translate(${cx * 14}px, ${cy * 8}px)`;
    if (nameRef.current) nameRef.current.style.transform = `translate(${cx * 5}px, ${cy * 3}px)`;
  }, []);
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    const SP = 28;
    let xs: number[] = [], ys: number[] = [], W = 0, H = 0;
    const init = () => {
      W = canvas.width = canvas.offsetWidth;
      H = canvas.height = canvas.offsetHeight;
      xs = []; ys = [];
      for (let x = SP / 2; x < W; x += SP)
        for (let y = SP / 2; y < H; y += SP) { xs.push(x); ys.push(y); }
    };
    const draw = () => {
      ctx.clearRect(0, 0, W, H);
      const lx = mousePos.x, ly = mousePos.y;
      for (let i = 0; i < xs.length; i++) {
        const dx = lx - xs[i], dy = ly - ys[i];
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      if (!deleting) {
        ...
        if (ci === phrase.length) {
          timeout = setTimeout(() => { deleting = true; timeout = setTimeout(step, 80); }, 2600);
          return;
        }
  ...
  return (
    <section ref={heroRef} className={styles.hero}>
      ...
      <div className={styles.glow} style={{ left: glowPos.x, top: glowPos.y }} aria-hidden="true" />
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.left}>
          <motion.div className={styles.badge} variants={fadeUp(0.08)} initial="hidden" animate="visible">
          ...
          >
            {"RIDHA".split("").map((char, i) => (
              <motion.span key={i} className={styles.ltr} variants={letterVariants(i)} initial="hidden" animate="visible">
                {char}
              </motion.span>
            ))}
            <motion.div
          ...
          <motion.div className={styles.specRow} variants={fadeUp(1.25)} initial="hidden" animate="visible">
            <span className={`${styles.spec} ${styles.specHi}`}>Software Developer</span>
          ...
          <motion.p className={styles.tagline} variants={fadeUp(1.45)} initial="hidden" animate="visible">
            {typeText}
            <span className={styles.caret} style={{ opacity: showCaret ? 1 : 0 }} aria-hidden="true" />
          </motion.p>
          ...
          <motion.div className={styles.socials} variants={fadeUp(1.85)} initial="hidden" animate="visible">
            <a href="https://github.com/ridhaak1" target="_blank" rel="noreferrer" className={styles.slink} aria-label="GitHub">
            ...
            <a href="https://linkedin.com/in/ridha-al-khaykanee-63928235a?" target="_blank" rel="noreferrer" className={styles.slink} aria-label="LinkedIn">
              <svg viewBox="0 0 24 24" aria-hidden="true">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
              </svg>
            </a>
            <a href="mailto:info.ridha.dev@gmail.com" className={styles.slink} aria-label="Email">
              <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div className={styles.center}>
          ...
          >
            ...
            <div className={styles.photoIn}>
              <img src={profilePhoto} alt="Ridha" className={styles.photoImg} />
            </div>
          </motion.div>
        ...
        <div className={styles.right}>
          <motion.div className={styles.stackHd} variants={fadeUp(1.1)} initial="hidden" animate="visible">
            Tech Stack
          </motion.div>
      ...
      <motion.div className={styles.scrollInd} variants={fadeUp(2.2)} initial="hidden" animate="visible" aria-hidden="true">
